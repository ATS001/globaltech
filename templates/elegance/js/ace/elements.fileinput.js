 ...
 (function($ , undefined) {
	...
	Ace_File_Input.prototype.apply_settings = function() {
		...
		if(this.settings.value_field != ''){
			...
			//if(format == 'file'){
				this.$element.parent().find('.btn_show').addClass('iframe_pdf').attr('rel',filename );;
			//}
		...
		for (var i = 0; i < files.length; i++) {
			...
			Ace_File_Input.prototype.show_file = function() {
				...
				if($link_file != this.settings.value_field){
                  ...
                  //$.colorbox({iframe:true, width:"90%", height:"95%",href:"?_tsk=shopdf&ajax=1&doc="+$link_file});
			...
			Ace_File_Input.prototype.remove_file = function() {
				...
				var $tester = true;
						...
						$.ajax({
							...
							data: '&del=1&f='+$('#'+this.$element.attr('id')+'-id').attr('value'),

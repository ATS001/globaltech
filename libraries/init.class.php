  ...
  */
  ...
  static public function crypt_tp($post, $value = null, $cryptage = 'C')
  {
    ...
    if($cryptage == 'C'){
      ...
      $post_value = $value;
      $hach       = MD5(uniqid(rand(), true));
      $post_crypt = MD5(base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($hach), $post_value, MCRYPT_MODE_CBC, md5(md5($hach)))));
      $retour = $post.'='.$post_value.'&'.$post.'h='.$hach.'&'.$post.'c='.$post_crypt;
      return $retour;

    }else{

      $post_value = Mreq::tp($post);
      $hach       = Mreq::tp($post.'h');
      $post_crypt = MD5(base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($hach), $post_value, MCRYPT_MODE_CBC, md5(md5($hach)))));

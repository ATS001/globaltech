...
class MySQL
{
  ...
  // Écriture des données
    ...
    public function Generate_reference($table, $abr) 
    {
    	...
    	(SELECT (SUBSTRING_INDEX(SUBSTRING_INDEX(a.reference, '-', - 1),'/', 1 ) * 1) AS id  FROM $table a ORDER BY id ) AS $table WHERE SUBSTRING_INDEX(a.reference, '/', - 1) = YEAR(CURDATE()) ORDER BY id;";
	...
	*/
	...
	public function RecordsArray($resultType = MYSQL_ASSOC) {
		...
		if ($this->last_result) {
			if (! mysql_data_seek($this->last_result, 0)) {
				...
				//while($member = mysql_fetch_object($this->last_result)){
				while ($member = mysql_fetch_row($this->last_result, $resultType)){
					$members[] = $member;
				}
	 ...
	 */
	public function RecordsSimplArray($resultType = MYSQL_NUM) {
		$this->ResetError();
		if ($this->last_result) {
			if (! mysql_data_seek($this->last_result, 0)) {
				$this->SetError();
	 ...
	 */
	public function RecordsSelectArray($resultType = MYSQL_ASSOC) {
		$this->ResetError();
		if ($this->last_result) {
			if (! mysql_data_seek($this->last_result, 0)) {
				$this->SetError();
	...
	public function After_update($table, $id, $arr_new, $arr_old) {
		...
		foreach ($aReturn as $key_g => $values) {
			...
			if (!$result = $this->InsertRow("espionnage_update", $arr_insert)) {
				exit($this->Error().' '.$this->BuildSQLInsert("espionnage_update", $arr_insert));
				$error = false;
			} else {
				$error = true;
			}

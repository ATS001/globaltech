...
class Mstock {
    ...
    var $id_stock       = null;        // stock ID append when request
	var $token            = null;        //user for recovery function
	var $stock_info     = array();     //Array stock all stock info
	...
	public function get_stock()
	{
		...
		$table WHERE  $table.id = ".$this->id_stock;
...
$values["mouvement"]       = MySQL::SQLValue($this->_data["mouvement"]);
			...
			if (!$result = $db->InsertRow($this->table, $values)) {
				...
				$this->last_id = $result;
				$this->log .='</br>Enregistrement  réussie '. $this->_data['stock'] .' - '.$this->last_id.' -';
				if(!Mlog::log_exec($this->table, $this->last_id, 'Création stock', 'Insert'))
                {
...
$values["mouvement"]       = MySQL::SQLValue($this->_data["mouvement"]);
			...
			if (!$result = $db->UpdateRows($this->table, $values, $wheres)) {
				...
				$this->last_id = $result;
				$this->log .='</br>Enregistrement  réussie '. $this->_data['stock'] .' - '.$this->last_id.' -';
				if(!Mlog::log_exec($this->table, $this->last_id, 'Modification stock', 'Update'))
                {
                    $this->log .= '</br>Un problème de log ';
                    $this->error = false;
                }
                //Esspionage
                if(!$db->After_update($this->table, $this->id_stock, $values, $this->stock_info)){
     ...
     * @return bol send to controller
     */
    public function valid_stock($etat)
    {
    	//Get existing data for row
		$this->get_stock();
    	...
    	if(!$result = $db->UpdateRows($this->table, $values, $wheres))
    	{
    		...
    		$this->error  = true;
    		if(!Mlog::log_exec($this->table, $this->last_id, 'Changement ETAT  stock', 'Update'))
    		{
    			$this->log .= '</br>Un problème de log ';
    			$this->error = false;
    		}
    ...
    public function delete_stock()
    {
    	...
    	$this->get_stock();
    	//Format where clause
    	$where['id'] = MySQL::SQLValue($id_stock);
    ...
    {
        if($this->stock_info[$key] != null)
        {
            echo $this->stock_info[$key];
        }else{
            echo "";
        }

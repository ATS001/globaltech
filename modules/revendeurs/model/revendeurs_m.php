...
class Mrev {
	...
	var $table = 'revendeurs'; //Main table of module
	...
	var $id_rev; // User ID append when request
	var $token; //user for recovery function
	var $rev_info; //Array stock all prminfo 
	...
	/**
	 * Get information for one revendeur
	 * @return [Array] [fill $this->rev_info]
	 */
	public function get_rev()
	{
		...
		$table,ref_ville v WHERE $table.ville=v.id and $table.id = ".$this->id_rev;
		if(!$db->Query($sql))
		{
			$this->error = false;
			$this->log  .= $db->Error();
			...
			if ($db->RowCount() == 0) {
				...
				$this->rev_info = $db->RowArray();
				$this->error = true;
			}	
		}
		//return Array rev_info
		if($this->error == false)
		{
			return false;
		}else{
    ...
    {
      if($this->rev_info[$key] != null)
      {
        echo $this->rev_info[$key];
      }else{
        echo "";
      }
	...
	{		
       //Befor execute do the multiple check
		...
		{
			$this->check_file('pj_image', 'Logo de la société.');
		}
		//Check $this->error (true / false)
		if($this->error == true){
			//Format values for Insert query 
			global $db;
			$values["type_rev"]    = MySQL::SQLValue($this->_data['type_rev']);
			...
			if (!$result = $db->InsertRow($this->table, $values)){
				...
				if($this->exige_pj)
				{
					$this->save_file('pj', 'Demande d\'agrément du revendeur '.$this->_data['denomination'], 'Document');
				}	
				if($this->exige_pj_image)
				{
					$this->save_file('pj_image', 'Logo de la société'.$this->_data['denomination'], 'Document');
				}				
	...
	{	
		...
		$this->last_id = $this->id_rev;	

		//Befor execute do the multiple check
		// check Dénomination revendeur
		$this->Check_exist('denomination', $this->_data['denomination'], 'Raison Sociale',  $this->id_rev);
		//Check Pièce d'identité' revendeur
		$this->Check_exist('piece_identite', $this->_data['piece_identite'], 'Registre Commerce', $this->id_rev);
		//Check N° d'agrément
		$this->Check_exist('num_agrement', $this->_data['num_agrement'], 'N° d\'agrément', $this->id_rev);			
		...
		if($this->exige_pj_image)
		{
			$this->check_file('pj_image', 'Le Logo de la société.', $this->_data['pj_image_id']);
		}
       ...
       //Check $this->error (true / false)
		if($this->error == true){
			...
			$wheres["id"]             = MySQL::SQLValue($this->id_rev);
    ...
    private function save_file($item, $titre, $type)
    {
    	...
    	$folder        = MPATH_UPLOAD.'revendeurs'.SLASH.$this->last_id;
    	...
    	if(!Minit::save_file_upload($temp_file, $new_name_file, $folder, $id_line, $title, 'revendeurs' ,$table, $column, $type, $edit = null))
    	{
    		$this->error = false;
     ...
     * @param  integer $etat [Etat du revendeur]
     * @return [bool]        [Retrn bol value read by controller]
     */
    public function valid_rev($etat = 0)
	{
		global $db;

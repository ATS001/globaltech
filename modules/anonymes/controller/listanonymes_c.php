...
<?php
	...
	$colms .= " prm_anonyme.id as id, ";
	$colms .= " prm_anonyme.titre as titre, ";
	$colms .= " prm_anonyme.longi as longi ,";
	$colms .= " prm_anonyme.latit as latit ,";
	$colms .= " prm_anonyme.technologie as technologie ,";


	//difine if user have rule to show line depend of etat 
	$where_etat_line = TableTools::where_etat_line('prm_anonyme', 'anonymes');
	
	//define notif culomn to concatate with any colms.
	//this is change style of button action to red
	$notif_colms = TableTools::line_notif('prm_anonyme', 'anonymes');
	
	//Format this column depend of data draw export or datatable.
	if(Mreq::tp('export')==1)
	{
		$colms .= " CONCAT(CASE prm_anonyme.etat 
                WHEN '0' THEN 'Inactif'
                WHEN '1' THEN 'Actif'
                ELSE ' ' END
                ,
	...
	if( !empty($params['search']['value']) or Mreq::tp('id_search') != NULL) 
	{
		...
		$where .=" ( prm_anonyme.id LIKE  $serch_value ";
		$where .=" OR prm_anonyme.titre LIKE $serch_value ";    
		$where .=" OR prm_anonyme.longi LIKE $serch_value ";
		$where .=" OR prm_anonyme.latit LIKE $serch_value ";
		$where .=" OR prm_anonyme.technologie LIKE $serch_value ";
		
        
        $where .=" OR CASE prm_anonyme.etat 
    ...
    if( Mreq::tp('export')==1 )
    {
    	...
    	$file_name = 'prm_anonyme';
    	$title     = 'Liste des anonymes ';
    	if(Mreq::tp('format')=='csv')
    	{
    		$header    = array('ID', 'Titre', 'Longitude', 'Latitude' ,'Technologie','Statut');
    		Minit::Export_xls($header, $file_name, $title);
    	}else{
    		$header    = array('ID'=>5, 'Titre'=>35, 'Longitude'=>15, 'Latitude'=>15 ,'Technologie'=>20,'Statut'=>10);

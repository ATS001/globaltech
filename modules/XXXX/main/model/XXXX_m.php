...
class MXXXX {
	...
	var $table            = 'visites';   //Main table of module
	var $last_id          = null;        //return last ID after insert command
	var $log              = null;        //Log of all opération.
	var $error            = true;        //Error bol changed when an error is occured
    var $id_XXXX       = null;        // XXXX ID append when request
	var $token            = null;        //user for recovery function
	var $XXXX_info     = array();     //Array stock all XXXX info
	...
	public function get_XXXX()
	{
		...
		$table WHERE  $table.id = ".$this->id_XXXX;
	...
	public function save_new_XXXX(){
		...
		if($this->error == true){
		    ...
		    $values["commerciale"]       = MySQL::SQLValue($this->_data["commerciale"]);
...
$values["commentaire"]       = MySQL::SQLValue($this->_data["commentaire"]);
			...
			if (!$result = $db->InsertRow($this->table, $values)) {
				...
				$this->last_id = $result;
				$this->log .='</br>Enregistrement  réussie '. $this->_data['XXXX'] .' - '.$this->last_id.' -';
				if(!Mlog::log_exec($this->table, $this->last_id, 'Création XXXX', 'Insert'))
                {
...
$values["commentaire"]       = MySQL::SQLValue($this->_data["commentaire"]);
			...
			if (!$result = $db->UpdateRows($this->table, $values, $wheres)) {
				...
				$this->last_id = $result;
				$this->log .='</br>Enregistrement  réussie '. $this->_data['XXXX'] .' - '.$this->last_id.' -';
				if(!Mlog::log_exec($this->table, $this->last_id, 'Modification XXXX', 'Update'))
                {
                    $this->log .= '</br>Un problème de log ';
                    $this->error = false;
                }
                //Esspionage
                if(!$db->After_update($this->table, $this->id_XXXX, $values, $this->XXXX_info)){
     ...
     * @return bol send to controller
     */
    public function valid_XXXX($etat)
    {
    	//Get existing data for row
		$this->get_XXXX();
    	...
    	if(!$result = $db->UpdateRows($this->table, $values, $wheres))
    	{
    		...
    		$this->error  = true;
    		if(!Mlog::log_exec($this->table, $this->last_id, 'Changement ETAT  XXXX', 'Update'))
    		{
    			$this->log .= '</br>Un problème de log ';
    			$this->error = false;
    		}
     ...
     * @return bol [Send to controller]
     */
    public function delete_XXXX()
    {
    	global $db;
    	$id_XXXX = $this->id_XXXX;
    	$this->get_XXXX();
    	//Format where clause
    	$where['id'] = MySQL::SQLValue($id_XXXX);
    ...
    {
        if($this->XXXX_info[$key] != null)
        {
            echo $this->XXXX_info[$key];
        }else{
            echo "";
        }

...
<?php
    ...
    // define used table.
	$tables .= " proforma, clients ";
    // define joint and rtable elation
	$joint .= " AND clients.id = proforma.id_client ";
	// set sherched columns.(the final colm without comma)
	$colms .= " proforma.id AS id_devi, ";
	$colms .= " DATE_FORMAT(proforma.date_proforma,'%d-%m-%Y') AS dat, ";
	$colms .= " proforma.reference AS ref, ";
	$colms .= " clients.denomination as client, ";
	$colms .= " REPLACE(FORMAT(proforma.totalht,0), ',', ' ') as montantht, ";
	$colms .= " REPLACE(FORMAT(proforma.totalttc,0), ',', ' ') as montantttc, ";
    
	
	
	//define notif culomn to concatate with any colms.
	//this is change style of button action to red
	$notif_colms = TableTools::line_notif_new('proforma', 'proforma');
	$colms .= $notif_colms;
	//difine if user have rule to show line depend of etat 
	$where_etat_line = TableTools::where_etat_line('proforma', 'proforma');
	...
	if( !empty($params['search']['value']) ) {
		...
		$where_s .=" AND ( proforma.reference LIKE '%".$serch_value."%' "; 
		$where_s .=" OR proforma.date_proforma LIKE '%".$serch_value."%' ";
		$where_s .=" OR clients.denomination LIKE '%".$serch_value."%' ";   
		$where_s .=" OR proforma.totalttc LIKE '%".$serch_value."%' ";
        $where_s .=" OR proforma.id LIKE '%".$serch_value."%' ";
        $where_s .= TableTools::where_search_etat('proforma', 'proforma', $serch_value);
    ...
    if( Mreq::tp('export')==1 )
    {
    	...
    	$file_name = 'proforma_list';
    	$title     = 'Liste proforma ';

<?php
  ...
  $main_table = 'd_proforma'; 
    // define used table.
  $tables .= " $main_table, produits ";
    // define joint and rtable elation
  $joint .= " WHERE produits.id = $main_table.id_produit AND $main_table.tkn_frm = '$tkn_frm'  ";
  // set  columns.(the final colm without comma)
  $colms .= " $main_table.id, ";
  $colms .= " $main_table.order item, ";
  
  $colms .= " $main_table.ref_produit, ";
  $colms .= " produits.designation, ";
  $colms .= " REPLACE(FORMAT($main_table.prix_unitaire,0),',',' '), ";
  $colms .= " $main_table.type_remise, ";
  $colms .= " REPLACE(FORMAT($main_table.remise_valeur,0),',',' '), ";
  $colms .= " REPLACE(FORMAT($main_table.qte,0),',',' '), ";
  $colms .= " REPLACE(FORMAT($main_table.total_ht,0),',',' '), ";
  $colms .= " REPLACE(FORMAT($main_table.total_tva,0),',',' '), ";
  $colms .= " REPLACE(FORMAT($main_table.total_ttc,0),',', ' '), ";
  $colms .= " ' ' as statut";
  ...
  if( !empty($params['search']['value']) ) {
    ...
    $where_s .=" AND ( produits.designation LIKE '%".$serch_value."%' ";    
    $where_s .=" OR $main_table.prix_ht LIKE '%".$serch_value."%' ";
    $where_s .=" OR $main_table.order LIKE '%".$serch_value."%' ";
    $where_s .=" OR $main_table.total_ttc LIKE '%".$serch_value."%' ";
    $where_s .=" OR $main_table.total_ht LIKE '%".$serch_value."%' ";
    $where_s .=" OR $main_table.total_ht LIKE '%".$serch_value."%' ";
    $where_s .=" OR $main_table.qte LIKE '%".$serch_value."%' ";
    $where_s .=" OR $main_table.ref_produit LIKE '%".$serch_value."%' )";

...
class Mticket_frs {
    ...
    var $id_ticket_frs       = null;        // ticket_frs ID append when request
	var $token            = null;        //user for recovery function
	var $ticket_frs_info     = array();     //Array stock all ticket_frs info
	...
	public function get_ticket_frs()
	{
		...
		$table WHERE  $table.id = ".$this->id_ticket_frs;
		...
		if(!$db->Query($sql))
		{
			...
			if ($db->RowCount() == 0) {
				...
				$this->ticket_frs_info = $db->RowArray();
				$this->error = true;
			}
...
$values["date_incident"]       = MySQL::SQLValue($this->_data["date_incident"]);
...
$values["observation"]       = MySQL::SQLValue($this->_data["observation"]);
			...
			if (!$result = $db->InsertRow($this->table, $values)) {
				...
				$this->last_id = $result;
				$this->log .='</br>Enregistrement  réussie '. $this->_data['ticket_frs'] .' - '.$this->last_id.' -';
				if(!Mlog::log_exec($this->table, $this->last_id, 'Création ticket_frs', 'Insert'))
                {
...
$values["observation"]       = MySQL::SQLValue($this->_data["observation"]);
			...
			if (!$result = $db->UpdateRows($this->table, $values, $wheres)) {
				...
				$this->last_id = $result;
				$this->log .='</br>Enregistrement  réussie '. $this->_data['ticket_frs'] .' - '.$this->last_id.' -';
				if(!Mlog::log_exec($this->table, $this->last_id, 'Modification ticket_frs', 'Update'))
                {
                    $this->log .= '</br>Un problème de log ';
                    $this->error = false;
                }
                //Esspionage
                if(!$db->After_update($this->table, $this->id_ticket_frs, $values, $this->ticket_frs_info)){
     ...
     * @return bol send to controller
     */
    public function valid_ticket_frs($etat)
    {
    	//Get existing data for row
		$this->get_ticket_frs();
    	...
    	if(!$result = $db->UpdateRows($this->table, $values, $wheres))
    	{
    		...
    		$this->error  = true;
    		if(!Mlog::log_exec($this->table, $this->last_id, 'Changement ETAT  ticket_frs', 'Update'))
    		{
    			$this->log .= '</br>Un problème de log ';
    			$this->error = false;
    		}
     ...
     * @return bol [Send to controller]
     */
    public function delete_ticket_frs()
    {
    	global $db;
    	$id_ticket_frs = $this->id_ticket_frs;
    	$this->get_ticket_frs();
    	//Format where clause
    	$where['id'] = MySQL::SQLValue($id_ticket_frs);
    ...
    {
        if($this->ticket_frs_info[$key] != null)
        {
            echo $this->ticket_frs_info[$key];
        }else{
            echo "";
        }
    ...
    public function g($key)
    {
        if($this->ticket_frs_info[$key] != null)
        {
            return $this->ticket_frs_info[$key];
        }else{
            return null;
        }

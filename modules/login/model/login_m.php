...
class MLogin 
{
	...
	private function get_user($email = null)
	{
		...
		$sql = "SELECT users_sys.* FROM 
		users_sys where  users_sys.nom = ".$this->id_user." OR users_sys.mail = ".$this->id_user;
		if($this->error == true)
		{
			if(!$db->Query($sql))
			{
	...
	private function check_pass()
	{
		...
		if ($this->user_info['pass'] != MD5($this->_data['pass'])){
			if (!$db->UpdateSinglRows('users_sys', 'ctc', $this->user_info['ctc'] + 1, $this->user_info['id'])) 
			{
				    //$db->Kill();
				$this->log .= $db->Error();
				$this->error = false;
	...
	{
		global $db;
		$sql_query = 'SELECT sign FROM services WHERE id = '.$this->user_info['service'];
	...
	private function delete_temp_folder($user = null, $session = null)
	{
		global $db;
		$username = $user;
		$sql_query = "SELECT id FROM session WHERE user = '".$username."' AND expir is NULL";
	...
	public function do_forgot()
	{
		...
		if ($this->error == true) {
            ...
            // Execute the insert
			if(!$result = $db->InsertRow("forgot", $values))
			{
				$this->error = false;
				$this->log  .= $db->Error();
			}

...
class Mvisites {
    ...
    var $id_visites = null;        // visites ID append when request
    var $token = null;        //user for recovery function
    var $visites_info = array();     //Array stock all visites info
     ...
     */
    public function save_new_visites() {
        ...
        if ($this->error == true) {
            ...
            if (!$result = $db->InsertRow($this->table, $values)) {
                ...
                if (!Mlog::log_exec($this->table, $this->last_id, 'Création visites', 'Insert')) {
    ...
    public function edit_visites() {
        ...
        if ($this->error == true) {
            ...
            if (!$result = $db->UpdateRows($this->table, $values, $wheres)) {
                ...
                $this->last_id = $result;
                $this->log .= '</br>Enregistrement  réussie '. $this->last_id . ' -';
                if (!Mlog::log_exec($this->table, $this->last_id, 'Modification visites', 'Update')) {
                    $this->log .= '</br>Un problème de log ';
                    $this->error = false;
                }
                //Esspionage
                if (!$db->After_update($this->table, $this->id_visites, $values, $this->visites_info)) {
     ...
     * @return bol send to controller
     */
    public function valid_visites($etat) {
        //Get existing data for row
        $this->get_visites();
        ...
        if (!$result = $db->UpdateRows($this->table, $values, $wheres)) {
            ...
            $this->error = true;
            if (!Mlog::log_exec($this->table, $this->last_id, 'Changement ETAT  visites', 'Update')) {
                $this->log .= '</br>Un problème de log ';
                $this->error = false;
            }
            //Esspionage
     ...
     * @return bol [Send to controller]
     */
    public function delete_visites() {
        global $db;
        $id_visites = $this->id_visites;
        $this->get_visites();
        //Format where clause
        $where['id'] = MySQL::SQLValue($id_visites);
    ...
    public function s($key) {
        if ($this->visites_info[$key] != null) {
            echo $this->visites_info[$key];
        } else {
            echo "";
        }
    }

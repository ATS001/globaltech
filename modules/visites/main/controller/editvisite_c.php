...
if (MInit::form_verif('editvisite', false)) {
    ...
    $edit_visites = new Mvisites($posted_data);

    //Set ID of row to update
    $edit_visites->id_visites = $posted_data['id'];

    //execute Update returne false if error
    if ($edit_visites->edit_visites()) {

        exit("1#" . $edit_visites->log);
    } else {

        exit("0#" . $edit_visites->log);
    }
}

//No form posted show view
view::load_view('editvisite');
?>
	...
	// Modul: contrats => Controller Liste
		...
		$joint .= " WHERE contrats.iddevis = devis.id  ";
		...
		if (Mreq::tp('export') == 1) {
		    ...
		    if (Mreq::tp('format') == 'csv') {
		        $header = array('ID', 'Référence', 'Devis', 'Date effet','Date fin', 'Statut');
		        Minit::Export_xls($header, $file_name, $title);
		    } elseif (Mreq::tp('format') == 'pdf') {
		        $header = array('ID' => 5, 'Référence' => 25, 'Devis' => 35, 'Date effet' => 15,'Date fin'=>15, 'Statut' => 20);

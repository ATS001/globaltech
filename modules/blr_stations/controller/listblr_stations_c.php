...
<?php
    ...
    // define used table.
	$tables .= " blr_stations,permissionnaires, ref_ville";
    // define joint and rtable elation
	$joint .= " WHERE   blr_stations.prm  = permissionnaires.id AND blr_stations.ville = ref_ville.id  ";
	// set sherched columns.(the final colm without comma)
	$colms .= " blr_stations.id as id, ";
	$colms .= " permissionnaires.r_social as rs, ";
	$colms .= " blr_stations.site as site ,";
	$colms .= " ref_ville.ville as ville ,";
	$colms .= " blr_stations.type_station as tstation ,";
	$colms .= " blr_stations.nbr_clients as nbrclients ,";


	//difine if user have rule to show line depend of etat 
	$where_etat_line = TableTools::where_etat_line('blr_stations', 'blr_stations');
	
	//define notif culomn to concatate with any colms.
	//this is change style of button action to red
	$notif_colms = TableTools::line_notif('blr_stations', 'blr_stations');
	
	//Format this column depend of data draw export or datatable.
	if(Mreq::tp('export')==1)
	{
		$colms .= " CONCAT(CASE blr_stations.etat 
                WHEN '0' THEN 'Inactif'
                WHEN '1' THEN 'Actif'
                ELSE ' ' END
                ,
	...
	if( !empty($params['search']['value']) or Mreq::tp('id_search') != NULL) 
	{
		...
		$where .=" OR blr_stations.id LIKE $serch_value ";    
		$where .=" OR ref_ville.ville LIKE $serch_value ";
		$where .=" OR blr_stations.site LIKE $serch_value ";
		$where .=" OR blr_stations.type_station LIKE $serch_value ";
		$where .=" OR blr_stations.nbr_clients LIKE $serch_value ";
    ...
    if( Mreq::tp('export')==1 )
    {
    	...
    	$file_name = 'blr_list';
    	$title     = 'Liste des stations BLR ';
    	if(Mreq::tp('format')=='csv')
    	{
    		$header    = array('ID','Permissionnaire' ,'Site','Ville','Type','Nbr clients','Statut');
    		Minit::Export_xls($header, $file_name, $title);
    	}else{
    		$header    = array('ID'=>5, 'Permissionnaire'=>25 ,'Site'=>20,'Ville'=>15,'Type'=>10,'Nbr clients'=>10,'Statut'=>10);

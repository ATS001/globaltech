<?php
if(MInit::form_verif('addblr_stations', false))
{
	$posted_data = array(
	  'prm'             => Mreq::tp('prm') ,	
  ...
  $new_blr_station = new  Mblr_stations($posted_data);
  $new_blr_station->exige_pj = true;
  
    //execute Insert returne false if error
  if($new_blr_station->save_new_blr_stations()){
    exit("1#".$new_blr_station->log);//Green message
  }else{
    exit("0#".$new_blr_station->log);//Red Message
  }

} else {
  //call form if no post
  view::load('blr_stations','addblr_stations');
}
?>
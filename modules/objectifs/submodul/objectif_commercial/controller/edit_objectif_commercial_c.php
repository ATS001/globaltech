...
if(MInit::form_verif('edit_objectif_commercial', false))
{
    ...
    if($posted_data["objectif"] == NULL OR !is_numeric($posted_data["objectif"])){
       ...
       //End check empty element
        $edit_objectifs = new  Mobjectif_commercial($posted_data);

        //Set ID of row to update
        $edit_objectifs->id_objectif_commercial = $posted_data['id'];
        
        //execute Update returne false if error
        if($edit_objectifs->edit_objectif_commercial()){
...
view::load_view('edit_objectif_commercial');

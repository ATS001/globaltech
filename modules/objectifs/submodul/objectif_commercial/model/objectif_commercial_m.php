...
class Mobjectif_commercial {
	...
	public function get_objectif_commercial()
	{
		global $db;
		...
		$table, commerciaux WHERE commerciaux.id = $table.id_commercial AND  $table.id = ".$this->id_objectif_commercial;
        //exit($sql);
		if(!$db->Query($sql))
		{
			$this->error = false;
			...
			if ($db->RowCount() == 0) {
				...
				$this->objectif_commercial_info = $db->RowArray();
				$this->error = true;
			}
     ...
     */
    public function Get_detail_objectif_commercial_show()
    {
        global $db;
        $table = $this->table;
	 ...
	 */
	public function save_new_objectif_commercial(){
    ...
    //Add all fields for the table
            ...
            if(!$result = $db->InsertRow($this->table, $values)) 
            {
                ...
                if(!Mlog::log_exec($this->table, $this->last_id, 'Création objectif_commercial', 'Insert'))
                {
	 ...
	 */
    public function edit_objectif_commercial(){
        ...
        if($this->error == true)
        {
            ...
            if (!$result = $db->UpdateRows($this->table, $values, $wheres))
            {
                ...
                if(!Mlog::log_exec($this->table, $this->last_id, 'Modification objectif_commercial', 'Update'))
                {
                    $this->log .= '</br>Un problème de log ';
                    $this->error = false;
                }
     ...
     * @return bol send to controller
     */
    public function valid_objectif_commercial()
    {
    	//Get existing data for row
		$this->get_objectif_commercial();
        ...
        if($etat_validation == $this->objectif_commercial_info['etat'])
        {
    	...
    	if(!$result = $db->UpdateRows($this->table, $values, $wheres))
    	{
    		...
    		$this->error  = true;
    		if(!Mlog::log_exec($this->table, $this->last_id, 'Changement ETAT  objectif_commercial', 'Update'))
    		{
    			$this->log .= '</br>Un problème de log ';
    			$this->error = false;
    		}
     ...
     * @return bol [Send to controller]
     */
    public function delete_objectif_commercial()
    {
    	global $db;
    	$id_objectif_commercial = $this->id_objectif_commercial;
    	$this->get_objectif_commercial();
    	//Format where clause
    	$where['id'] = MySQL::SQLValue($id_objectif_commercial);
    ...
    {
        if($this->objectif_commercial_info[$key] != null)
        {
            echo $this->objectif_commercial_info[$key];
        }else{
            echo "";
        }
    ...
    public function g($key)
    {
        if($this->objectif_commercial_info[$key] != null)
        {
            return $this->objectif_commercial_info[$key];
        }else{
            return null;
        }
    ...
    public function get_list_factures_for_objectif()
    {
        ...
        $id_objectif   = $this->id_objectif_commercial;
    ...
    public function get_list_devis_for_objectif()
    {
        ...
        INNER JOIN `commerciaux`ON (`commerciaux`.`id` IN (REPLACE((REPLACE(`devis`.`id_commercial`,'[\"','')),'\"]','')) )         
        WHERE encaissements.etat IN(1, 0) 
        AND encaissements.`date_encaissement` BETWEEN '$date_s' AND '$date_e' 
        AND devis.etat <> 200 AND devis.id_commercial LIKE '%\"$id_commercial\"%' ";

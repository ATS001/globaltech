<?php
if(MInit::form_verif('editgsm', false))//If form is Posted do Action else rend empty form
{
...
//Check if have error return Red message  
  ...
  $edit_gsm = new  Mgsm($posted_data);
  //Exige PJ formulaire
  $edit_gsm->exige_pj     = true;
  //Set ID of line
  $edit_gsm->id_gsm = $posted_data['id'];
  //execute Insert returne false if error
  if($edit_gsm->edit_gsm()){
    exit("1#".$edit_gsm->log);//Return green message
  }else{
    exit("0#".$edit_gsm->log);//return Red Message
  }
} else {
  //If no form posted call Edit gsm form view
  view::load('gsm','editgsm');
}

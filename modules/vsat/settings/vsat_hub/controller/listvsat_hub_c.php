...
<?php
    ...
    // define used table.
	$tables .= " vsat_hub,ref_pays";
	$colms .= " vsat_hub.id AS id_vsat_hub, ";
	$colms .= " vsat_hub.operateur as op, ";
	$colms .= " ref_pays.pays as p, ";
	$colms .= " vsat_hub.ville_hub as vh, ";
	$colms .= " vsat_hub.email_hub as eh, ";
	$joint .= " WHERE   vsat_hub.pays_hub  = ref_pays.id";
	

	//difine if user have rule to show line depend of etat 
	$where_etat_line = TableTools::where_etat_line('vsat_hub', 'vsat_hub');
	
	//define notif culomn to concatate with any colms.
	//this is change style of button action to red
	$notif_colms = TableTools::line_notif('vsat_hub', 'vsat_hub');
	
	//Format this column depend of data draw export or datatable.
	if(Mreq::tp('export')==1)
	{
		$colms .= " CONCAT(CASE vsat_hub.etat 
                WHEN '0' THEN 'Inactif'
                WHEN '1' THEN 'Actif'
                ELSE ' ' END
                ,
	...
	if( !empty($params['search']['value']) ) {
	    ...
	    $where .= $joint == NULL? " WHERE " : " AND ";
            ...
            $where .=" (vsat_hub.operateur LIKE '%".$serch_value."%' ";
			$where .=" OR ref_pays.pays LIKE '%".$serch_value."%' ";
			$where .=" OR vsat_hub.ville_hub LIKE '%".$serch_value."%' ";
			$where .=" OR vsat_hub.email_hub LIKE '%".$serch_value."%' ";
            $where .=" OR vsat_hub.id LIKE '%".$serch_value."%' ";
            $where .=" OR CASE vsat_hub.etat 
                WHEN '0' THEN 'Inactif'
                WHEN '1' THEN 'Active'
    ...
    if( Mreq::tp('export')==1 )
    {
    	...
    	{
    		$header    = array('ID', 'Opérateur', 'Pays','Ville','E-mail');
    		Minit::Export_xls($header, $file_name, $title);
    	}else{
    		$header    = array('ID'=>10, 'Opérateur'=>20, 'Pays'=>25,'Ville'=>20,'E-mail'=>25);

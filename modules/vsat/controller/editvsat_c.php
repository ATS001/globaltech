<?php
if(MInit::form_verif('editvsat', false))//If form is Posted do Action else rend empty form
{
  ...
  /*if($posted_data['revendeur'] == null OR !MInit::exist_select('revendeur', $posted_data['revendeur'])){
  ...
  $edit_vsat = new  Mvsat($posted_data);
  //Exige PJ formulaire
  $edit_vsat->exige_pj     = true;
  //Set ID of line
  $edit_vsat->id_vsat = $posted_data['id'];
  //execute Insert returne false if error
  if($edit_vsat->edit_vsat()){
    exit("1#".$edit_vsat->log);//Return green message
  }else{
    exit("0#".$edit_vsat->log);//return Red Message
  }
} else {
  //If no form posted call Edit VSAT form view
  view::load('vsat','editvsat');
}

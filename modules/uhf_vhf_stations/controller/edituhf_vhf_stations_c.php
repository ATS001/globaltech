...
if (MInit::form_verif('edituhf_vhf_stations', false)) {//If form is Posted do Action else rend empty form
    ...
    //End Checker
    //Call Model
    $new_uhf_vhf_stations = new Muhf_vhf_stations($posted_data);
    $new_uhf_vhf_stations->exige_pj = true;

    $new_uhf_vhf_stations->id_uhf_vhf_stations = $posted_data['id'];
    //execute Edit returne false if error
    if ($new_uhf_vhf_stations->edit_uhf_vhf_stations()) {
        exit("1#" . $new_uhf_vhf_stations->log); //Green message
    } else {
        exit("0#" . $new_uhf_vhf_stations->log); //Red message
    }
//Call View if no POST
} else {
    view::load('uhf_vhf_stations', 'edituhf_vhf_stations');
}
?>
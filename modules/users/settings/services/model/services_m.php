...
class Mservice {
	...
	public function get_service()
	{
		...
		$sql = "SELECT services.* FROM 
		services WHERE services.id = ".$this->id_service;
		if(!$db->Query($sql))
		{
			$this->error = false;
			$this->log  .= $db->Error();
	...
	public function save_new_service(){
		...
		if($this->error == true){
			...
			if (!$result = $db->InsertRow("services", $values)) {
	...
	public function update_service()
	{
		...
		if($this->error == true){
			...
			if (!$result = $db->UpdateRows("services", $values, $wheres)) {
...
public function delete_service()
    {
    	...
    	if(!$db->DeleteRows('services',$where))
    	{
    		$this->log .= $db->Error();
			$this->error = false;
...
public function valid_service()
	...
	{
		global $db;
		$sql = "SELECT sign FROM services WHERE id = $id_service ";

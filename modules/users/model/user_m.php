...
class Musers {
	...
	var $table           = 'users_sys'; //Main table of module
  ...
  var $user_connexion_history = null; //Array stock all user connexion history
	...
	public function get_user()
	{
		...
		$sql = "SELECT users_sys.*, services.service as service_user FROM 
		users_sys, services WHERE users_sys.service = services.id AND  users_sys.id = ".$this->id_user;
		if(!$db->Query($sql))
		{
			$this->error = false;
			$this->log  .= $db->Error();
 ...
 */
  public function get_activities()
  {
    ...
    users_sys u, sys_log l WHERE u.nom = l.user_exec AND  u.id =$this->id_user order by l.`datlog` desc limit 0,6";
    if(!$db->Query($sql))
    {
      $this->error = false;
      $this->log  .= $db->Error();
  ...
  public function get_connexion_history()
  {
    ...
    public function save_new_user()
    {
        ...
        //Service si existe
    	$this->check_non_exist('services', 'id', $this->_data['service'], 'Service');
		//Test for password
    ...
    public function edit_user()
    {
      ...
      if($this->error == true)
      {
        ...
        if (!$result = $db->UpdateRows("users_sys", $values, $wheres)) {
				//$db->Kill();
         $this->log .= $db->Error();
         $this->error == false;
    ...
    //exit($db->BuildSQLDelete('rules_action', $where));
		...
		{
			$this->error = false;
			$this->log  .= '</br>Impossible de supprimer les anciens rules '.$db->Error().' '.$db->BuildSQLDelete('rules_action', $where);
		}
      ...
      if($this->error == true)
      {
       ...
       if( !$result = $db->UpdateRows("users_sys", $value , $where)){
        ...
        $wheres["token"] =  MySQL::SQLValue($token);
                // Execute the update
        if(!$results = $db->UpdateRows("forgot", $values , $wheres))
        {
 ...
 public function change_pass()
 {
  ...
  $password = $db->QuerySingleValue("SELECT pass FROM users_sys where id='$user_id' and etat = 1");
